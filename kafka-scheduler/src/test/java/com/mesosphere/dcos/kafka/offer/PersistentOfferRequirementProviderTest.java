...
public class PersistentOfferRequirementProviderTest {
  ...
  public void testNewRequirement() throws Exception {
    ...
    Assert.assertEquals(2, cmd.getUrisList().size());

    Assert.assertEquals(KafkaTestUtils.testJavaUri, cmd.getUrisList().get(0).getValue());
    Assert.assertEquals(KafkaTestUtils.testExecutorUri, cmd.getUrisList().get(1).getValue());
    ...
    Assert.assertEquals("while true; do echo `date`': hello' >> $MESOS_SANDBOX/hello_world.txt; sleep 60; done", kafkaTaskData.getValue());
